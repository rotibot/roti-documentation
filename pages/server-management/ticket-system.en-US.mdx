...
Ticket transcripts within R.O.T.I. provide a complete record of conversations that occurred within a support ticket channel.  This is valuable for review, accountability, or referencing past interactions.

### Generating Transcripts

There are two main ways to trigger the creation of a ticket transcript:

- **Ticket Close Command**: When you run the `/ticket close` command to close a ticket, a transcript will automatically be generated.
- **Close with Transcript Button**: By clicking the `Delete & Transcript` button, a transcript will be automatically generated, and the ticket will be closed. 

![Ticket Close](./img/ticket-close.png)

### Accessing Transcripts

- `Direct Message`: When a transcript is generated, R.O.T.I. will send a direct message to the user who created the ticket. This message will contain the complete transcript as a downloadable file.

- `Logging Channel`: You have the option to configure a designated channel on your server using the `/ticket config` command, where ticket transcripts are automatically logged for archival or review purposes.

<Callout type="info">
Transcripts include all messages sent within the ticket channel, along with timestamps and the usernames of those who participated.
</Callout>
...
![Ticket Stats Command](./img/ticket-stats.png)
